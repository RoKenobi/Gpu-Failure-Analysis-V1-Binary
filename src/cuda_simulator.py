...
def simulate_gpu_telemetry(voltage_arr, temp_arr, memory_arr, status_arr, rng_states, n_events):
    idx = cuda.grid(1)
    if idx < n_events:
        # Generate random numbers directly on the GPU using xoroshiro128p
        v_noise = xoroshiro128p_normal_float32(rng_states, idx) * 0.05
        t_noise = xoroshiro128p_normal_float32(rng_states, idx) * 5.0
        m_noise = xoroshiro128p_normal_float32(rng_states, idx) * 10.0
